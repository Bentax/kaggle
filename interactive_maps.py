...
###########################
### The data. Now, we'll add some crime data to the map! We won't focus on the data loading step. Instead, you can imagine you are at a point where you already have the data in a pandas DataFrame crimes. The first five rows of the data are shown below.
...
###########################
## Plotting points. To reduce the amount of data we need to fit on the map, we'll (temporarily) confine our attention to daytime robberies.
...
#########################
## Bubble maps. A bubble map uses circles instead of markers. By varying the size and color of each circle, we can also show the relationship between location and two other variables.
...
#######################
## Heatmaps. To create a heatmap, we use folium.plugins.HeatMap(). This shows the density of crime in different areas of the city, where red areas have relatively more criminal incidents.
...
########################
## Choropleth maps. To understand how crime varies by police district, we'll create a choropleth map.
